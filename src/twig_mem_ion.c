...
static int ion_alloc(int ion_dev_fd, size_t size) {
    ...
    int ret = ioctl(ion_dev_fd, ION_IOC_ALLOC, &alloc);
    if (ret < 0) 
        return -1;
...
static uint32_t ion_get_phys_addr(int ion_dev_fd, int handle) {
	...
	int ret = ioctl(ion_dev_fd, ION_IOC_CUSTOM, &custom_data);
    if (ret < 0)
		return 0x0;
...
static int ion_map(int ion_dev_fd, int handle) {
    if (ion_dev_fd < 0 || handle < 0)
        return -1;
    ...
    int ret = ioctl(ion_dev_fd, ION_IOC_MAP, &fd_data);
    if (ret < 0)
        return -1;
...
static void ion_free(int ion_dev_fd, int handle) {
    if (ion_dev_fd < 0 || handle < 0)
        return;
    ...
    ioctl(ion_dev_fd, ION_IOC_FREE, &handle_data);
}

static void ion_flush_cache(int ion_dev_fd, void *addr, size_t size) {
    if (ion_dev_fd < 0 || addr == NULL || size <= 0)
        return;
...
static twig_mem_t *twig_allocator_ion_mem_alloc(twig_allocator_t *allocator, size_t size) {
    ...
    mem->handle = ion_alloc(allocator->ion_dev_fd, size);
    if (mem->handle < 0)
        goto err_free;

    mem->pub_mem.phys = ion_get_phys_addr(allocator->ion_dev_fd, mem->handle);
    if (!mem->pub_mem.phys)
        goto err_ion_free;
...
static void twig_allocator_ion_mem_flush(twig_allocator_t *allocator, twig_mem_t *pub_mem) {
    ...
    struct ion_mem *mem = (struct ion_mem *)pub_mem;
    ion_flush_cache(allocator->ion_dev_fd, pub_mem->virt, pub_mem->size);
}
...
twig_allocator_t *twig_allocator_ion_create(void) {
    ...
    allocator->ion_dev_fd = open("/dev/ion", O_RDONLY);
    if (allocator->ion_dev_fd == -1) {
        free(allocator);
        return NULL;
    }
